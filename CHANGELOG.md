...
revisiones y en la etiqueta anotada de Git. La v1.1 se **actualiza en sitio**: la re-inspección de la
PE5 cerró siete defectos residuales sobre el mismo número de versión, en lugar de abrir una v1.2 que
habría multiplicado versiones sin cambio de alcance. El apartado de la PE5, más abajo, registra esas
correcciones por separado. Se emplea la numeración de la asignatura: `v1.0` es el
documento de la Entrega 2A y `v1.1` la versión resultante del CCB. Su equivalencia con las
revisiones internas del ERS (3.0 y 3.1) está declarada en el `README.md` y en el
propio ERS.

---

## [v1.1 — actualización de la PE5] — 2026-08-17

Cierre de los **siete defectos residuales** detectados por la re-inspección de la Unidad V sobre el
ERS ya corregido. No se abre una versión nueva: el alcance y el catálogo de requisitos no cambian, de
modo que la v1.1 se actualiza en sitio y el PDF publicado al cierre de la PE4 queda sustituido por el
que generan las fuentes de `01_ERS/`. Etiqueta de Git: `baseline-final`.

El recuento de defectos residuales es el dato empírico de la métrica **M6 = 0,11** de la auditoría de
calidad. Registro completo en `02_Auditoria/AnexoA_auditoria_calidad.xlsx`, hoja `Reinspeccion_M6`.

### Añadido

- Diagramas de flujo de datos de la §8, ausentes en la versión publicada en la PE4.
- Historias de usuario de `RF-36` y `RF-37`, que el informe de la PE4 declaraba aplicadas pero que el
  PDF publicado no incorporaba.
- §9, especificación de los dos componentes de inteligencia artificial: 6 requisitos funcionales
  (`RF-IA-01` a `RF-IA-06`) y 12 no funcionales (`RNF-IA-01` a `RNF-IA-12`), con umbrales de
  rendimiento, equidad y explicabilidad, clasificación de riesgo conforme al Reglamento (UE)
  2024/1689 y plan de monitoreo de deriva.
- Especificación textual de ocho casos de uso (`CU-11` a `CU-18`), que elevó la métrica M1b del
  55,6 % al 100 %.
- Apéndice F con 37 criterios de aceptación en formato Dado--Cuando--Entonces (`CB-01` a `CB-37`),
  que elevó la verificabilidad M3 del 39,3 % al 100 %.

### Cambiado

- Matriz de trazabilidad: se resuelve la contradicción entre las 48 y las 52 filas declaradas, y se
  amplía a **73 filas** con la cadena Fuente → RF → CU → Clase → Proceso → Estado → BDD → CP.
- Cobertura del MVP: se recalcula con los dos denominadores declarados, corrección que el informe de
  la PE4 daba por aplicada y que el PDF publicado no reflejaba.
- Distinción expresa entre clase de usuario y parte interesada, que elevó la cobertura de actores
  M1c del 75,0 % al 100 %.

### No corregido

- **M6 = 0,11 se reporta sin corregir.** La métrica mide qué proporción de defectos escapó a la
  inspección de la PE4: su numerador es un hecho histórico y cerrar los siete defectos mejora el
  documento pero no altera el valor. La acción registrada es de proceso —ampliar la lista de
  verificación a apéndices y conteos globales— y consta como elemento `S2` de la retrospectiva.
- (DEF-25, menor) `RF-09` sigue sin enumerar las etapas del ciclo del insecto, por la misma razón
  declarada en la v1.1.

---

## [v1.1] — 2026-08-05, actualizada el 2026-08-17
...
defectos en `02_Auditoria/AnexoB_registro_defectos.xlsx`; texto antes/después de cada corrección en
la §5 del informe.
